    ...
    if let Ok(temp_dir) = windows::shadow_copy::temp_folder(".tmp", "", 10) {
      ...
      if result.is_ok() {
        ...
        for suffix in ["-wal", "-shm", "-journal"] {
          ...
          if sidecar.exists() {
            let sidecar_result =
              windows::shadow_copy::shadow_copy(sidecar.clone(), temp_dir.clone().to_path_buf());
            log::debug!("shadow copy sidecar {:?}: {:?}", sidecar, sidecar_result);
          }
        }
...
fn query_cookies(
  ...
  while let Some(row) = rows.next()? {
    ...
    let decrypted_value = match decrypt_encrypted_value(value, &encrypted_value, keys.to_owned()) {
      Ok(value) => value,
      Err(err) => {
        log::warn!("Skipping cookie {} for {}: {}", name, host_key, err);
        continue;
      }
    };
